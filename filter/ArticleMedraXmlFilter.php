...
class ArticleMedraXmlFilter extends O4DOIXmlFilter
{
    ...
    public function createArticleNode(DOMDocument $doc, Submission|Galley $pubObject): DOMElement
    {
        ...
        if (is_a($pubObject, 'Submission')) {
            ...
            if ($cache->isCached('articles', $publication->getData('submissionId'))) {
                ...
                if ($article && $article->getData('status') === Submission::STATUS_PUBLISHED) {
                    $cache->add($article, null);
                }
            }
    ...
    public function createContentItemNode(DOMDocument $doc, Issue $issue, Submission $article, ?Galley $galley, string $pubObjectDoi, array $objectLocalePrecedence): DOMElement
    {
        ...
        if (!empty($issueDoi)) {
            $relatedIssueIds[self::O4DOI_ID_TYPE_DOI] = $issueDoi;
        }
        ...
        if (!$galley) { // if exporting object is an article
            ...
            foreach ($galleysByArticle as $relatedGalley) {
                ...
                if (!empty($galleyDoi)) {
                    $relatedGalleyIds[self::O4DOI_ID_TYPE_DOI] = $galleyDoi;
                }
            ...
            $doi = $article->getCurrentPublication()->getDoi();
            if (!empty($doi)) {
                $relatedArticleIds[self::O4DOI_ID_TYPE_DOI] = $doi;
            }
